...
- TypeScript 统一使用 `5.9.x`。
- ESLint 统一使用 `10.4.x`。
- `@typescript-eslint/parser` 与 `@typescript-eslint/eslint-plugin` 统一使用 `8.59.x`。
- Prettier 统一使用 `3.8.x`。
- Husky 统一使用 `9.1.x`。
- lint-staged 统一使用 `16.4.x`。
- 版本升级优先跟随同主版本 patch 或 minor，不要在不同 app 和 package 中各自漂移。
...
- `strict` 必须为 `true`。
- `strictNullChecks`、`noImplicitAny`、`noUncheckedIndexedAccess` 不允许被应用私自关闭。
...
- DTO 类属性在 strict 模式下应使用明确初始化策略，例如 `!` 或默认值，不允许通过关闭 `strictPropertyInitialization` 规避。

## 4.1 tsconfig 约束

- 全仓基础配置统一来源于 `packages/tsconfig`。
- React 应用和 `packages/ui` 必须继承 `packages/tsconfig/react.json`。
- Node/Nest 相关工程优先继承 `packages/tsconfig/node.json` 或 `base.json`，禁止在应用内复制第二套 TS 基线。
- 共享包若不需要 Node 类型，不要为了省事继承带 `types: ["node"]` 的配置。
...
- 通用视觉基础能力优先放 `packages/ui` 和 `packages/design-tokens`，不要在业务页面重复发明按钮、卡片、badge、颜色和圆角体系。
...
### 6.5 design-tokens 规范

- 颜色、字号、间距、圆角、阴影等视觉基础规则必须统一沉淀到 `packages/design-tokens`。
- 业务页面禁止直接硬编码新的品牌色、圆角值、阴影值或大面积重复字号定义。
- Tailwind theme 中扩展的颜色和 spacing 应以 token 为来源，不允许 UI token 与 Tailwind token 各自维护两套真值。
- 新增视觉 token 时，必须同时说明用途、适用范围，以及是否面向 C 端、管理台或全局共享。
- 若某样式只在一个组件中短期使用，先判断是否真的是 token；不要把一次性样式滥抽成全局设计资产。

### 6.6 图标规范

- 全站统一使用 `lucide-react` 作为默认图标库。
- 页面和业务组件中不要混用其他图标库，除非有明确业务理由且经过文档说明。
- 图标优先通过共享 UI 层或本地包装组件导出，统一控制尺寸、描边宽度、颜色和交互态。
- 禁止在页面中到处散落不一致的 icon size，如 15px、17px、19px 这类无规则值；优先使用约定尺寸，如 14、16、18、20、24。
- 图标颜色应优先继承文本语义色或 token 色，不要直接写临时十六进制颜色。
- 用于按钮、输入框、状态提示的图标必须与交互区域一起考虑可点击范围和可访问性。
...
- 废弃依赖要及时清理。
- Lint、格式化、类型检查相关依赖统一放在根目录管理，不在各 app 中各自安装第二套。
...
- 提交前默认会触发 Husky hooks，不能依赖“先提交再修”。

## 14.1 ESLint 与 Prettier 规范

- ESLint 负责代码质量、潜在 bug、未使用变量、类型导入和基础一致性规则。
- Prettier 只负责格式化，不承载语义级代码质量判断。
- 根目录统一使用 flat config：`eslint.config.mjs`。
- 可复用规则放在 `packages/eslint-config`，业务 app 只消费，不各自维护大段重复规则。
- `consistent-type-imports` 作为默认规则，纯类型导入必须使用 `import type`。
- 公共组件中的 `button` 必须显式声明 `type`，避免表单上下文中的隐式提交。

## 14.2 Husky 与 lint-staged 规范

- 根目录必须启用 `Husky + lint-staged`。
- `pre-commit` 默认执行：
  - `prettier --write`
  - `eslint --fix`
- `pre-push` 默认执行：
  - `pnpm lint`
  - `pnpm typecheck`
- 轻量检查放在 `pre-commit`，全量检查放在 `pre-push` 或 CI，不要把所有重量级任务都堆到一次 commit 上。
- 如果 hooks 经常被绕过，优先修复速度和配置问题，而不是把 hooks 直接关掉。
...
- 涉及视觉基础规则时已检查是否应进入 `design-tokens`。
- 涉及图标时默认使用 `lucide-react`，且尺寸、颜色、描边风格保持一致。
- 提交前能通过 Husky 与 lint-staged 的本地检查。
